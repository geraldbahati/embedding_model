...
def extract_text_from_excel_as_json(file_stream):
    """
    Extract text from Excel file stream (.xls or .xlsx) and convert to a nested JSON format
    wrapped in 'timetable' key.
    """
    json_output = {}
...
def extract_text_from_excel_as_csv(file_stream):
    ...
    for sheet_name in xls.sheet_names:
        ...
        csv_content = df.to_csv(
            index=False
        )  # Convert dataframe to CSV format, without writing row indices
        csv_texts.append(csv_content)

    return "\n".join(
        csv_texts
    )  # Join multiple sheets' CSV content with a newline separator
...
def main():
    ...
    with st.sidebar:
        ...
        pdf_docs = st.file_uploader(
            ...
            type=["pdf", "docx", "doc", "pptx", "xls", "xlsx"],
